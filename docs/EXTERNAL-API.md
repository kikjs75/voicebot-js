...
| `RTZR_CLIENT_SECRET` | RTZR STT | developers.rtzr.ai 콘솔 |
| `GOOGLE_APPLICATION_CREDENTIALS` | Google Cloud TTS | docs/GCP-TTS-SETUP.md 참조 |
...
전화 수신 (G.711, 8kHz)
  │
  ▼  [real profile]
STT: RTZR WebSocket 스트리밍
  → PCM 8kHz 16bit 전송 → "EOS" 종료
  ← final:true 텍스트
  │
  ▼
...
TTS: Google Cloud TTS REST
  → LINEAR16, sampleRateHertz=8000
  ← base64 디코딩 → 8kHz WAV binary
  │
  ▼
전화 송출 (G.711, 8kHz)
```
